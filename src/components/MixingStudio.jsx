...
function MixingStudio({ allPaints }) {
  ...
  return (
    <div className="bg-gradient-mixing-studio rounded-2xl shadow-2xl p-8 border border-purple-100">
      <h2 className="text-3xl font-bold mb-6 text-gradient-purple">Paint Mixing Studio</h2>
            ...
            {selectedPaints.map(paint => (
              <div key={paint.id} className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-shadow border border-gray-200">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gradient-red-pink text-white rounded-xl hover:opacity-90 transition-all shadow-md hover:shadow-lg text-sm font-medium"
                >
                  Remove
                </button>
              </div>
      ...
      {/* Mixed Color Preview */}
      {mixedColor && (
        <div className="mb-8 bg-gradient-preview p-8 rounded-2xl border-purple-glow shadow-xl">
          <h3 className="text-2xl font-bold mb-6 text-gradient-purple">Color Preview</h3>
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="relative group gradient-border-animated">
              <div
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="inline-block bg-white px-6 py-3 rounded-xl shadow-lg mb-4">
                ...
                <div className="text-3xl font-mono font-bold text-gradient-purple">{mixedColor}</div>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-96 overflow-y-auto p-1">
            {filteredAvailablePaints.slice(0, 20).map(paint => (
              <button
                ...
                className="flex items-center gap-2 p-3 bg-white hover:bg-purple-50 rounded-xl transition-all text-left border border-gray-200 hover:border-purple-300 shadow-sm hover:shadow-md"
              >
                <div
